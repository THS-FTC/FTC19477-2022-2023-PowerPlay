...
public class Auto_Blue_Carousel extends LinearOpMode {
    //variables
    ...
    public void runOpMode() {
        ...
        if (opModeIsActive()) {
            ...
            //find in middle mark
                ...
                motors_set_position(-450, 450, 450, -450, motors_velocity);//move to the left to get to the 1st barcode pos
                sleep(2500);
                if (find_marker() == true){
                    marker_pos = 1;
...
void put_cargo(){//3 - put cargo in correct position indicated by shipping element
    ...
    sleep(1700);//wait 1.7 sec
    //motors_set_position(-880, 880, -880, 880, motors_velocity);//turn left around perpendicular to warehouse
    //sleep(1500);//wait 1.5 sec
}


void duck_carousel(){//1 - deliver duck from carousel
    motors_set_position(2350, -2350, -2350, 2350, motors_velocity);//encoder values to get to the carousel - moving right
    sleep(4100);//wait 4.1 sec for motors to run
    carousel_turn();//on
}

void warehouse_park(){//4 - end of auto period - park in storage space
    motors_set_position(900, 900, 900, 900, motors_velocity);//move forward to align with storage
    sleep(1500);//wait 1.5 sec
    motors_set_position(600, -600, -600, 600, motors_velocity);//move right into warehouse
