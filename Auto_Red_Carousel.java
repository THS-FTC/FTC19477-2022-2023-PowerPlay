/*
- RED TEAM CODE; if blue, select the proper file
- 1 is first
...
public class Auto_Red_Carousel extends LinearOpMode {
    //variables
...
void put_cargo(){//3 - put cargo in correct position indicated by shipping element
    if (marker_pos == 2){
        motors_set_position(1100, -1100, -1100, 1100, motors_velocity);//move left and stop before where the alliance hub should be
        sleep(2000);//wait 1.85 sec
    }
    else if (marker_pos != 2){
        motors_set_position(800, -800, -800, 800, motors_velocity);//move left and stop before where the alliance hub should be
        sleep(1400);//wait 1 sec
    }
    arm(marker_pos);
    sleep(1000);
    motors_set_position(500, 500, 500, 500, motors_velocity);//move forward to the hub
    sleep(1700);//wait 1.7 sec
    intake();
    motors_set_position(-500, -500, -500, -500, motors_velocity);//move back to align with warehouse
    sleep(1700);//wait 1.7 sec
    motors_set_position(880, -880, 880, -880, motors_velocity);//turn right around perpendicular to warehouse
    sleep(1500);//wait 1.5 sec
}


void duck_carousel(){//1 - deliver duck from carousel
    motors_set_position(-2450, -2450, -2450, -2450, motors_velocity);//encoder values to get to the carousel - moving backwards
    sleep(4100);//wait 4.1 sec for motors to run
    carousel_turn();//on
}

void warehouse_park(){//4 - end of auto period - park in storage space
    motors_set_position(-900, 900, 900, -900, motors_velocity);//move left to align with storage
    sleep(1500);//wait 1.5 sec
    motors_set_position(-600, -600, -600, -600, motors_velocity);//move back into warehouse
...
//other function control!!!    
    void carousel_turn(){
        CRServo_1.setPower(-1.0);//change to negative for auto red
